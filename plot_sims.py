...
def plot_first_four_sources(fits_files, output_file):
    ...
    # Create a list of axes for each subplot
    axes = []
    for i in range(4):
        axes.append([fig.add_subplot(4, 3, i * 3 + j + 1, projection=WCS(fits.open(fits_files[i])['INDEX_MAP'].header)) for j in range(3)])
    ...
    for i, fits_file in enumerate(fits_files[:4]):
        ...
        ax1 = axes[i][0]
        im1 = ax1.imshow(-1 * spectral_index_map, origin='lower', cmap='turbo_r', interpolation='none', vmin=0.5, vmax=4)
        ax1.contour(image, levels=contour_levels, colors='black', alpha=0.8, transform=ax1.get_transform(wcs), linewidths=0.6)
        ...
        ax2.contour(image, levels=contour_levels, colors='black', alpha=0.8, transform=ax2.get_transform(wcs), linewidths=0.6)
        ...
        ax3.contour(image, levels=contour_levels, colors='black', alpha=0.8, transform=ax3.get_transform(wcs), linewidths=0.6)
        ...
        # Plot host galaxy coordinates
        for ax in [ax1, ax2, ax3]:
            if host_coords is not None:
                for j, c in enumerate(host_coords, start=1):
                    ax.plot(c.ra.deg, c.dec.deg, marker='x', color='black', transform=ax.get_transform('fk5'), markersize=10)
                    ax.text(c.ra.deg, c.dec.deg, f'   {j}', color='black', transform=ax.get_transform('fk5'), fontsize=10, ha='left', va='top')

    # Add common colorbars at the top for each column
    # Column 1: Spectral Index
    cbar1 = fig.colorbar(im1, ax=[axes[j][0] for j in range(4)], orientation='horizontal', shrink=0.99, pad=0.1, aspect=30, location='top')

    # Column 2: Spectral Index Error
    cbar2 = fig.colorbar(im2, ax=[axes[j][1] for j in range(4)], orientation='horizontal', shrink=0.99, pad=0.1, aspect=30, location='bottom')

    # Column 3: Chi2red map
    cbar3 = fig.colorbar(im3, ax=[axes[j][2] for j in range(4)], orientation='horizontal', shrink=0.99, pad=0.1, aspect=30, location='bottom')
...
# def plot_first_four_sources(fits_files, output_file):
#     fig = plt.figure(figsize=(15, 0.8*20 * np.sqrt(2)))  
#     labels = ['(a)', '(b)', '(c)', '(d)']
#     all_axes =[]
# 
#     for i, fits_file in enumerate(fits_files[:4]):
...
#         ax3.coords[1].set_ticklabel_visible(False)
#         ax3.coords[1].set_axislabel('')
#         #cbar3 = fig.colorbar(im3, ax=ax3, orientation='horizontal', shrink=0.69, pad=0.01,aspect=30, location='top')
#         #cbar3 = fig.colorbar(im3, ax=ax3, orientation='vertical', shrink=0.99, pad=0.01,aspect=30)
#         # Plot hg coords
#         axes = [ax1, ax2, ax3]
#         for ax in axes:
#             wcsaxes.add_beam(ax, header=header,alpha=0.9,pad=0.65,frame=True, facecolor='none', edgecolor='black')
#             if host_coords is not None:
...
#             all_axes.append(ax)
#     
#     cbar1 = fig.colorbar(im1, ax=[all_axes[j][0] for j in range(4)], orientation='horizontal', shrink=0.69, pad=0.1,aspect=30,location='top')
#     cbar2 = fig.colorbar(im2, ax=[all_axes[j][1] for j in range(4)], orientation='horizontal', shrink=0.69, pad=0.1,aspect=30,location='top')
#     cbar3 = fig.colorbar(im3, ax=[all_axes[j][2] for j in range(4)], orientation='horizontal', shrink=0.69, pad=0.1,aspect=30, location='top')
#     fig.tight_layout(w_pad=0, h_pad=0.2)
#     plt.savefig(output_file)
#     plt.close(fig)

def plot_remaining_sources(fits_files, output_file):
    fig = plt.figure(figsize=(15, 0.9*15 * np.sqrt(2)))  # Made it match paper size #fig = plt.figure(figsize=(15, 0.8*3/4*15 * np.sqrt(2)))  # Made it match paper size
    labels = ['(e)', '(f)', '(g)','(h)']
    for i, fits_file in enumerate(fits_files[4:]):
        with fits.open(fits_file) as hdul:
            header = hdul['INDEX_MAP'].header
        ...
        ax1.coords[0].set_axislabel('R.A. (J2000)')
        ax1.coords[1].set_axislabel('Dec. (J2000)')
        cbar1 = fig.colorbar(im1, ax=ax1, orientation='vertical', shrink=0.86, pad=0.01,aspect=30)
        # Label
        ax1.text(0.02, 0.95, labels[i], transform=ax1.transAxes, fontsize=14, fontweight='bold', va='top', ha='left')
        # spectral index error map
        ax2 = fig.add_subplot(4, 3, i * 3 + 2, projection=wcs)
        im2 = ax2.imshow(spectral_index_error_map, origin='lower', cmap='turbo', interpolation='none', norm=LogNorm())
        ax2.contour(image, levels=contour_levels, colors='black', alpha=0.9, transform=ax2.get_transform(wcs), linewidths=0.6)
        ax2.tick_params(direction='in')
        if i == 0:
            ax2.set_title('Spectral Index Error')
        ax2.coords[0].set_axislabel('R.A. (J2000)')
        ax2.coords[1].set_ticklabel_visible(False)
        ax2.coords[1].set_axislabel('')
        cbar2 = fig.colorbar(im2, ax=ax2, orientation='vertical', shrink=0.86, pad=0.01,aspect=30)
        
        # chi2red map
        ax3 = fig.add_subplot(4, 3, i * 3 + 3, projection=wcs)
        im3 = ax3.imshow(chi2red_map, origin='lower', cmap='inferno', interpolation='none', norm=LogNorm())
        ax3.contour(image, levels=contour_levels, colors='black', alpha=0.9, transform=ax3.get_transform(wcs), linewidths=0.6)
        ax3.tick_params(direction='in')
        if i == 0:
            ax3.set_title(r'$\chi^2_{red}$')
        ax3.coords[0].set_axislabel('R.A. (J2000)')
        ax3.coords[1].set_ticklabel_visible(False)
        ax3.coords[1].set_axislabel('')
        cbar3 = fig.colorbar(im3, ax=ax3, orientation='vertical', shrink=0.86, pad=0.01,aspect=30)
    ...
    plt.tight_layout(pad=1, h_pad=0.0, w_pad=0.0)
    plt.savefig(output_file)
    plt.close(fig)
